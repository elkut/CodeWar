...
function foo(a, b) {
    ...
    const sortedB = b.map((value, index) => ({ value, index }))
        .sort((a, b) => a.value - b.value);

    const leftover = [];
    const shuffledArr = new Array[n];

    let j = 0;
    for (const x in sortedA) {
        if (j < n && sortedB[j].value < x) {
            shuffledArr[sortedB[j].index] = x
            j++
        }
        else {
            leftover.push(x);
    ...
    let k = 0
    for (let i = 0; i < n; i++) {
        if (shuffledArr[i] === 'undefined') {
            shuffledArr[i] = leftover[k++];
        }
    }
